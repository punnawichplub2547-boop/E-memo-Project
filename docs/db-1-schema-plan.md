...
Write persistence is DB-2. The first DB-2 slices persist newly created memos from `ADD_MEMO` and approval advancement from `ADVANCE_STEP`; return, reject, read, and revision mutations still update memory only. `MemoProvider` hydrates its initial state from `GET /api/memos` when available. The SA's Phase 2 roadmap is the governing design reference.
