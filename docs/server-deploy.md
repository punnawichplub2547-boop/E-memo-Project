...
The app will be available on port `3000`. Docker Compose also starts a MySQL 8 container. The app loads initial memo data from `GET /api/memos` when the DB is available. New memo creation and approval advancement are persisted to DB; later workflow mutations still remain in-memory prototype state.
...
- New memo creation and approval advancement are persisted to DB.
- Return, reject, read, and revision mutations are still prototype in-memory state.
