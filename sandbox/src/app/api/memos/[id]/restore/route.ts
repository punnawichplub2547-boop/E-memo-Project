...
import { buildRestoreMemoPayload, type SoftDeleteMemoBody } from "@/lib/db-memo-write";

export const dynamic = "force-dynamic";

type MemoIdRow = RowDataPacket & { id: number };

// Restore a soft-deleted memo: clears memos.deleted_at and appends a "restore" audit row.
export async function POST(
  ...
  try {
    ...
    const { memoUpdate, workflowAction } = buildRestoreMemoPayload({ ...body, actorName });
    ...
    console.error("[POST /api/memos/[id]/restore]", error);
    return NextResponse.json({ error: "Unable to restore memo" }, { status: 500 });
  } finally {
    connection?.release();
  }
}
