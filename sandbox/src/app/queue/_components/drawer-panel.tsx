...
import { MemoBodyBlocksSection } from "./memo-body-blocks-section";
import { RequestItemsSection, PriceComparisonSection } from "./memo-summary-tables";
...
export function DrawerPanel({
  ...
  // Request Items / Price Comparison, split into their own components
  // (memo-summary-tables.tsx) to keep this file under the 700-line
  // guardrail. Built once here so standard mode (rendered at their usual
  // document position below) and free-form mode (passed into
  // MemoBodyBlocksSection as systemSlots) render the exact same JSX — the
  // V2 §2 discount/VAT rules and formatting live only there, never copied.
  const requestItemsSection = <RequestItemsSection memo={memo} />;
  const priceComparisonSection = <PriceComparisonSection memo={memo} />;
  ...
  return (
    ...
    >
      ...
      <div className="em-drawer-body">
        ...
        {/* 5. Request items — standard mode renders requestItemsSection in place;
            free-form mode renders the whole body-block list here instead (which
            includes requestItemsSection/priceComparisonSection wherever the
            requester placed a "system" block for them, and only there — see
            memo-body-blocks-section.tsx). */}
        {memo.formMode === "freeform" ? (
          <MemoBodyBlocksSection
            memo={memo}
            systemSlots={{
              priceComparison: priceComparisonSection,
              requestItems: requestItemsSection,
            }}
          />
        ) : (
          requestItemsSection
        ...
        {/* 6. Price Comparison Summary — standard mode only. In free-form mode
            this is already rendered above (inside MemoBodyBlocksSection, only
            where the requester placed a "system" block for it); rendering
            priceComparisonSection here too would show the table twice. */}
        {memo.formMode !== "freeform" && priceComparisonSection}
