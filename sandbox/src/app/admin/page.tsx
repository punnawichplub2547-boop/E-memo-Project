...
import {
  ...
  IconRefresh, IconX, IconCheck, IconKey, IconSettings, IconReturn,
...
type Tab = "db-users" | "memos" | "master-data" | "audit" | "issues" | "system";
...
export default function AdminPage() {
  ...
  async function handleDeleteIssue(id: number) {
    try {
      ...
      setIssueError("ลบไม่สำเร็จ กรุณาลองใหม่");
    }
  }
      ...
      <div className="em-work">
        ...
        <div className="em-content">
          ...
          <div className="em-admin-tabs" style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: "1px solid var(--border)", paddingBottom: 0 }}>
            {([ ["db-users", IconUsers, "Registered Users"], ["memos", IconFileText, "Memos"], ["master-data", IconTag, "Master Data"], ["audit", IconHistory, "Audit Log"], ["issues", IconBell, "แจ้งปัญหา"], ["system", IconSettings, "System"] ] as const).map(([t, Icon, label]) => (
                      ...
                      <tr key={r.id} style={{ borderBottom: "1px solid var(--border)" }}>
                        ...
                        <td style={{ padding: "10px 14px", whiteSpace: "nowrap" }}>
                          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                            ...
                            {confirmDeleteIssueId === r.id ? (
                              <>
                                <button className="em-btn" style={{ padding: "5px 9px", fontSize: 11.5, color: "#DC2626" }}
                                  onClick={() => handleDeleteIssue(r.id)}>ยืนยันลบ</button>
                                <button className="em-btn" style={{ padding: "5px 9px", fontSize: 11.5 }}
                                  onClick={() => setConfirmDeleteIssueId(null)}>ยกเลิก</button>
                              </>
                            ) : (
                              <button className="em-btn" title="ลบถาวร" style={{ padding: "5px 9px", fontSize: 11.5, color: "#DC2626" }}
                                onClick={() => setConfirmDeleteIssueId(r.id)}><IconTrash size={13} /></button>
                            )}
                          </div>
                        </td>
                      </tr>
