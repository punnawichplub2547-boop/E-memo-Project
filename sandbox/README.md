...
- DB-2 has started: new memo creation (`ADD_MEMO`), approval advancement (`ADVANCE_STEP`), return-for-revision (`RETURN_MEMO`), and rejection (`REJECT_MEMO`) are persisted to MySQL; read and revision mutations still stay in memory.
